...
export function Dashboard() {
  ...
  // Sync event bus — fires after offline queue flush / manual pull
  useEffect(() => {
    const u1 = onSyncEvent('sync:done',     load)
    const u2 = onSyncEvent('cache:updated', load)
    return () => { u1(); u2() }
  }, [load])

  // Supabase Realtime — fires on live DB changes (online only)
  useEffect(() => {
