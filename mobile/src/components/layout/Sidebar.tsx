...
export function Sidebar({ collapsed }: { collapsed: boolean }) {
  ...
  return (
    <aside className={`fixed top-0 left-0 h-screen bg-white border-r border-gray-100 flex flex-col z-30 transition-all duration-200 ${collapsed ? 'w-16' : 'w-56'}`}>
      {/* Logo */}
      <div className={`flex items-center h-16 border-b border-gray-100 flex-shrink-0 ${collapsed ? 'justify-center px-2' : 'px-4 gap-2.5'}`}>
        <img
          src="/brand/logo.png"
          alt="TEN"
          className={`object-contain flex-shrink-0 ${collapsed ? 'h-7 w-7' : 'h-7'}`}
          onError={(e) => { (e.target as HTMLImageElement).style.display = 'none' }}
        />
        {!collapsed && (
          <div className="min-w-0 border-l border-gray-100 pl-2.5">
            <p className="text-xs font-black text-gray-800 leading-none">TenPOS</p>
