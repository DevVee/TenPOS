...
export function StockAdjustments() {
  ...
  const filtered = adjustments.filter((a) => {
    ...
    return a.product_name?.toLowerCase().includes(q) || a.reason?.toLowerCase().includes(q) || a.user_name?.toLowerCase().includes(q)
  })
              ...
              {filtered.length === 0 ? (
                ...
                filtered.map((a) => (
                  <tr key={a.id} className="table-row">
                    ...
                    <td className={`px-4 py-3 text-sm font-semibold text-right ${Number(a.quantity) > 0 ? 'text-green-600' : 'text-brand'}`}>
                      {Number(a.quantity) > 0 ? '+' : ''}{a.quantity}
                    </td>
                    <td className="px-4 py-3 text-sm text-gray-500 hidden md:table-cell">{a.reason}</td>
                    <td className="px-4 py-3 text-sm text-gray-500 hidden sm:table-cell">{a.user_name}</td>
