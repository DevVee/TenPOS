...
import { Plus, MapPin, Monitor, Edit, Loader2, AlertCircle } from 'lucide-react'
...
export function Branches() {
  ...
  const openEdit = (b: Branch) => {
    ...
    setForm({
      ...
      terminal_count: String(b.terminal_count ?? 1),
    })
    setSaveError('')
    setModal(true)
  }
            ...
            <div key={b.id} className="card p-4">
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <span className="text-gray-600">
                    {b.terminal_count ?? 0} POS Terminal{(b.terminal_count ?? 0) !== 1 ? 's' : ''}
                  </span>
                </div>
              </div>
              <div className="mt-3 pt-3 border-t border-gray-50 flex justify-between items-center">
                <div>
                  ...
                  <p className="text-sm font-medium text-gray-700 mt-0.5">{b.manager_name || '—'}</p>
                </div>
                <span className="text-xs text-gray-400">{b.staff_count ?? 0} staff</span>
              </div>
            </div>
          ))}
          {branches.length === 0 && (
