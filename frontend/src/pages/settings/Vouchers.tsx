...
const BLANK = {
  code: '', discountType: 'percentage' as 'percentage' | 'fixed', value: '', minOrder: '0',
...
export function Vouchers() {
  ...
  const openEdit = (v: Voucher) => {
    ...
    setForm({
      ...
      minOrder:     String(v.min_order),
      maxUses:      String(v.max_uses ?? 100),
      expiry:       v.expiry ? v.expiry.slice(0, 10) : '',
      active:       v.active,
    })
    setSaveError('')
    setShowModal(true)
          ...
          <div className="divide-y divide-gray-100">
            {vouchers.length === 0 ? (
              ...
              vouchers.map((v) => {
                ...
                const exhausted = v.max_uses !== null && v.uses_count >= v.max_uses
                        ...
                        <div className="min-w-0">
                          ...
                          <div className="flex items-center gap-3 mt-1.5 flex-wrap">
                            ...
                            <div className="flex items-center gap-1 text-xs text-gray-400">
                              ...
                              <span className={expired ? 'text-red-400' : ''}>{v.expiry ? v.expiry.slice(0, 10) : '—'}</span>
                            </div>
                            {v.min_order > 0 && (
                              <span className="text-xs text-gray-400">Min ₱{v.min_order}</span>
                            )}
                          </div>
                        </div>
                      </div>
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-medium text-gray-700 mb-1.5">
                {form.discountType === 'percentage' ? 'Discount (%)' : 'Discount (₱)'} <span className="text-brand">*</span>
              </label>
              <div className="relative">
                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm">{form.discountType === 'percentage' ? '%' : '₱'}</span>
