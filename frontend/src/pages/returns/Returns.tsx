...
interface Txn {
  ...
  total: string; cashier_name: string; created_at: string
}
interface TxnItem { id: string; product_name: string; quantity: number; total: string }
...
export function Returns() {
  ...
  const filtered = entries.filter((r) => {
    ...
    return r.receipt_no?.toLowerCase().includes(q) || r.cashier_name?.toLowerCase().includes(q)
  })
              ...
              {filtered.length === 0 ? (
                ...
                filtered.map((r) => (
                  <tr key={r.id} className="table-row">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-500 hidden sm:table-cell">{r.cashier_name}</td>
        ...
        <div className="space-y-4">
          ...
          {voidTxn && (
            <div className="bg-gray-50 rounded-xl p-3 text-sm space-y-1">
              ...
              <div className="flex justify-between"><span className="text-gray-500">Cashier</span><span>{voidTxn.cashier_name}</span></div>
