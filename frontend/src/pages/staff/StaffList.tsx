                ...
                filtered.map((s) => {
                  ...
                  return (
                    <tr key={s.id} className="table-row cursor-pointer" onClick={() => navigate(`/staff/${s.id}`)}>
                      ...
                      <td className="px-4 py-3 text-sm text-gray-500 hidden sm:table-cell">{s.branch_name ?? '—'}</td>
                      <td className="px-4 py-3 text-xs text-gray-400 hidden md:table-cell">
                        {s.last_login
                          ? new Date(s.last_login).toLocaleString('en-PH', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })
                          : 'Never'}
                      </td>
                      <td className="px-4 py-3">
