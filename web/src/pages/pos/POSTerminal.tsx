...
export function POSTerminal() {
  ...
  const {
    ...
    clearCart, searchQuery, setSearch, cartSubtotal, syncStatus,
  } = usePOSStore()
      ...
      try {
        ...
        setProducts(
          data.filter((p) => p.active).map((p) => ({
            ...
            barcode: p.barcode ?? '', category: p.category_name || 'Other',
            price: p.price, cost: p.cost, stock: p.stock,
            reorderPoint: p.reorder_point, imageUrl: p.image_url,
            ...
            color: p.color, weightGrams: p.weight_grams, lengthCm: p.length_cm,
    ...
    <div className="flex flex-col h-screen overflow-hidden" style={{ background: '#F5F7FA' }}>
      ...
      <header className="h-14 bg-white border-b border-gray-200 flex items-center justify-between px-4 flex-shrink-0 z-10">
        ...
        <div className="flex items-center gap-2">
          <div className={`hidden sm:flex items-center gap-1.5 text-xs font-medium px-2.5 py-1.5 border rounded-lg transition-colors ${
            syncStatus === 'offline'
...
function ProductInfoModal({ product, onClose, onAddToCart }: ProductInfoModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-xl shadow-modal w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden z-10 animate-modal">
        ...
        <div className="flex flex-1 min-h-0 overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
            ...
            {(product.brand || product.material || product.color || dimStr || product.weightGrams) && (
              <div>
                <p className="section-label mb-1.5">Details</p>
                <div className="space-y-1.5">
            ...
            {product.variants && product.variants.length > 0 && (
              <div>
                <p className="section-label mb-1.5">Variants</p>
...
function CartPanel({
  ...
  removeFromCart, updateQty, clearCart, navigate, onCheckout,
}: CartPanelProps) {
  return (
    <>
      {/* Header */}
      ...
      <div className="flex-1 overflow-y-auto px-3 py-2 space-y-2">
        {cart.length === 0 ? (
          ...
          cart.map((item) => {
            // Use product+variant composite key to avoid collision when the
            // same product is added twice with different variants.
            const cartKey   = `${item.product.id}_${item.variant?.id ?? 'base'}`
            const linePrice = item.product.price + (item.variant?.priceAdjustment ?? 0)
            const lineTotal = linePrice * item.quantity - item.discount
            return (
              <div key={cartKey} className="bg-gray-50 rounded-lg overflow-hidden border border-gray-100">
                <div className="flex items-start gap-2.5 p-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-medium text-gray-800 line-clamp-1 leading-tight">
                      {item.product.name}
                      {item.variant && (
                        <span className="text-gray-400 font-normal"> · {item.variant.value}</span>
                      )}
                    </p>
                ...
                <div className="flex items-center gap-2 px-3 pb-3">
                  ...
                  <div className="flex items-center bg-white border border-gray-200 rounded-lg overflow-hidden">
                    ...
                    <QtyInput value={item.quantity} onChange={(n) => updateQty(item.product.id, n, item.variant?.id)} />
                    <button
                      onClick={() => updateQty(item.product.id, item.quantity + 1, item.variant?.id)}
                  ...
                  {/* Discount — keyed on product+variant to prevent collision */}
                  <div className="flex items-center gap-1 flex-1">
                    <span className="text-[10px] text-gray-400 font-medium flex-shrink-0">Disc</span>
                    <input
                      type="number"
                      min="0"
                      value={discountInput[cartKey] ?? item.discount}
                      onChange={(e) =>
                        setDiscountInput((d) => ({ ...d, [cartKey]: e.target.value }))
                      }
                      onBlur={(e) => {
                        const v = parseFloat(e.target.value) || 0
                        usePOSStore.getState().applyDiscount(item.product.id, v, item.variant?.id)
                      }}
